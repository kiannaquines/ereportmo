...
<body>
    ...
    <p>Generated by: <strong>{{ $generated_by }}</strong> ({{ ucfirst($user_role) }})</p>
    
    @if($is_admin)
        <p style="background-color: #d1fae5; padding: 8px; border-left: 4px solid #10b981; margin: 10px 0;">
            <strong>Admin View:</strong> Showing all incidents across all offices and municipalities
        </p>
    @elseif($user_office)
        <p style="background-color: #dbeafe; padding: 8px; border-left: 4px solid #3b82f6; margin: 10px 0;">
            <strong>{{ $user_office }} View:</strong> 
            Showing incidents for {{ $user_office }}
            @if($user_municipality)
                in {{ $user_municipality }}
            @endif
        </p>
    @endif
